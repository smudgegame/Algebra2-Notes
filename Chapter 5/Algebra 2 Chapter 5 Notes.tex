...
\vspace{-0.35cm}
\noindent \underline{Step 3}: Press \tcbox[size=small, on line]{2nd} \tcbox[size=small, on line]{$y=$} (\texttt{STAT PLOT} button). Select \texttt{1:Plot1$\ldots$Off}. Arrow over \texttt{On} and select it. Arrow down to \texttt{YList:} and press \tcbox[size=small, on line]{2nd} \tcbox[size=small, on line]{STAT} (\texttt{LIST}) and select \texttt{L2}. 
